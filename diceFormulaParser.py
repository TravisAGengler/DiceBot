...
def _resolve_expression(expression):
	...
	for child in expression.children:
		...
		if operation == "basic_dice":
			...
			return _roll_explode_dice(int(child.children[0]), int(child.children[1]), int(child.children[2]))
			...
			return {'total' : val1['total'] ** val2['total'], 'individual' : val1['individual'] + val2['individual']}
...
def _do_test_case(formula, expected, seed):
	...
	if result != expected:
		print('Error on unit test {}: actual value {} is not expected value {}'.format(formula, result, expected))
		return False
	return True
...
def _do_unit_tests():
	cases = [
		{
			'formula' : "1d20",
			'expected' : {'individual': ['15'], 'total': 15}
		},
		{
			'formula' : "1d20 + 10",
			'expected' : {'individual': ['15'], 'total': 25}
		},
		{
			'formula' : "2d6 + 1d20",
		...
		{
			'formula' : "2*1d8 + 3",
			'expected' : {'individual': ['8'], 'total': 19}
		},
		{
			'formula' : "3df",
			'expected' : {'individual': ['-', '-', '+'], 'total': -1}
		},
		{
			'formula' : "10df + 1d10",
			'expected' : {'individual': ['-', '-', '+', '+', '.', '.', '-', '+', '+', '+', '3'], 'total': 5}
		},
		{
			'formula' : "(1d2*3) + (1d3*4)",
		...
		{
			'formula' : "8d6t5",
			'expected' : {'individual': ['4', '4', '1', '1', '**16**', '2', '1', '1'], 'total': 30, 'successes' : 1}
		},
		{
			'formula' : "10d6t20",
			'expected' : {'individual': ['4', '4', '1', '1', '**16**', '2', '1', '1', '2', '5'], 'total': 37, 'successes' : 0}
		},
		{
			'formula' : "4d2t3",
			'expected' : {'individual': ['**5**', '1', '**3**', '1'], 'total': 10, 'successes' : 2}
		}
	]
